...
<div class="container" style="margin-top: 150px; margin-bottom: 80px;">
    ...
    <div class="mb-4">
        <a href="{{ route('mobil.index') }}" class="btn btn-teal-outline">
        ...
        <div class="card-body p-0">
            <div class="row g-0">
                <div class="col-md-6 bg-light d-flex align-items-center justify-content-center p-5">
                    @if($mobil->gambar)
                        <img src="{{ asset('storage/'.$mobil->gambar) }}" class="img-fluid rounded" alt="{{ $mobil->nama }}" style="max-height: 400px;">
                    @else
                ...
                <div class="col-md-6 p-5 position-relative">
                    ...
                    <div class="position-absolute top-0 end-0 m-4 d-flex gap-2" style="z-index: 10;">
                        <a href="{{ route('mobil.edit', $mobil->id) }}" class="btn btn-warning text-white btn-sm px-3 shadow-sm">
                            <i class="fas fa-edit"></i> Edit
                        </a>

                        <form action="{{ route('mobil.destroy', $mobil->id) }}" method="POST" onsubmit="return confirm('Yakin hapus mobil ini?');">
                            @csrf
                            @method('DELETE')
                    ...
                    <h5 class="text-muted text-uppercase letter-spacing-1 mt-2">{{ $mobil->tipe }}</h5>
                    <h1 class="fw-bold text-dark mb-3">{{ $mobil->nama }}</h1>
                    ...
                    <div class="row mt-4 mb-4">
                        <div class="col-6 mb-3">
                            ...
                            <span class="fw-bold">{{ $mobil->tahun_produksi }}</span>
                        </div>
                        <div class="col-6 mb-3">
                            <small class="text-muted d-block">Warna</small>
                            <span class="fw-bold">{{ $mobil->warna }}</span>
                        </div>
                        <div class="col-6 mb-3">
                            <small class="text-muted d-block">Plat Nomor</small>
                            <span class="badge bg-secondary">{{ $mobil->plat_nomor }}</span>
                        </div>
                        <div class="col-6 mb-3">
                    ...
                    <div class="d-grid gap-2">
                        ...
                        @else
                            ...
                            <button class="btn btn-secondary btn-lg text-capitalize" disabled>
                                {{ $mobil->status == 'disewa' ? 'Unit Sedang Disewa' : 'Unit Sedang Maintenance' }}
                            </button>
                        @endif
                    </div>
                </div>
